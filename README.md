...
The Repeller Method should allow one to make an infinite ensemble. The next phase of this research project would be to combine predictions with a moving average (EMA is probably right). The weights that each new models repels from is less clear. The naive idea would be to use a standard or moving average as well, to avoid the time complexity of trying to repel away from several weight matrices. Other ideas for weighting could be by accuracy on the training data. First over fit, then regularize.
